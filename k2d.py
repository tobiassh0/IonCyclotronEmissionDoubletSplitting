...
if __name__=='__main__':
    ...
    try:
        ...
        Z = make2D(kpara,kperp,omegas_arr/wcalpha,rowlim=(0,kparamax*k0),collim=(0,kperpmax*k0),limits=False,bins=(200,200))
    ...
    ax[0].contour(Z,levels=np.arange(0,kperpmax+1,1),origin='lower',aspect='auto',colors='k',extent=[0,kperpmax,0,kparamax])

    # compare to high freq equivalent
    tkperp = np.linspace(0,kperpmax,500)*k0
    tkpara = np.linspace(0,kparamax,500)*k0
    KPARA,KPERP=np.meshgrid(tkpara,tkperp)
    K2 = KPARA**2 + KPERP**2
    ...
    ax[1].contour(np.sqrt(W2)/wcalpha,levels=np.arange(0,kperpmax+1,1),origin='lower',colors='w',extent=[0,kperpmax,0,kparamax])
    ...
    plt.colorbar(im, cax=cbar, orientation='vertical', spacing='proportional')
